...
ggplot(data = diamonds.sample) + geom_point(mapping = aes(x = carat, y = price, fill = clarity, color = cut), alpha = 0.1)
...
diamond.summary <- diamonds %>% group_by(clarity) %>% summarise(mean = mean(price), sd = sd(price), se = sd / length(price))

# Then draw the plot. The error bars should stretch from the mean-error to the mean+error.
ggplot(data = diamond.summary) + geom_bar(mapping = aes(x = clarity, y = mean, fill = clarity), stat = 'identity') +   geom_errorbar(data = diamond.summary, aes(ymin=(mean-se), ymax=(mean+se)))
#### why not work
